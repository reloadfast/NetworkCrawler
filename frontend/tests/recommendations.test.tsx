...
import { useRecommendations, useDeviceRecommendations } from '../src/hooks/useRecommendations'
...
const rec1: Recommendation = {
  ...
  steps: ['Step one: disable telnet service', 'Step two: verify port 23 closed'],
  effort: 'low',
  impact: 'high',
...
describe('useRecommendations', () => {
  ...
  it('appends severity filter to query string', async () => {
    const fetchMock = mockFetch([])
    vi.stubGlobal('fetch', fetchMock)
    const { result } = renderHook(() => useRecommendations({ severity: 'critical' }))
    await waitFor(() => expect(result.current.loading).toBe(false))
    const url = fetchMock.mock.calls[0][0] as string
...
describe('RecommendationsPage', () => {
  it('renders list of recommendations', async () => {
    ...
    await waitFor(() => expect(screen.getByText('Disable Telnet')).toBeInTheDocument())
  ...
  it('renders sort select with default severity option', async () => {
    ...
    await waitFor(() => expect(screen.getByText('Disable Telnet')).toBeInTheDocument())
    const select = screen.getByRole('combobox', { name: /Sort recommendations/i })
  ...
  it('shows device link for each recommendation', async () => {
    ...
    await waitFor(() => expect(screen.getByText('Device #10')).toBeInTheDocument())
  })
})
...
describe('RecommendationDetailPage', () => {
  ...
  it('renders title, description, and steps', async () => {
    ...
    renderDetail('1')
    await waitFor(() =>
      expect(screen.getByRole('heading', { name: 'Disable Telnet' })).toBeInTheDocument(),
    )
    expect(screen.getByText('Telnet transmits data in plaintext.')).toBeInTheDocument()
    expect(screen.getByText('Step one: disable telnet service')).toBeInTheDocument()
    expect(screen.getByText('Step two: verify port 23 closed')).toBeInTheDocument()
  })
  ...
  it('shows device link', async () => {
    vi.stubGlobal('fetch', mockFetch([rec1]))
    renderDetail('1')
    await waitFor(() => expect(screen.getByText(/Device #10/i)).toBeInTheDocument())
  })
