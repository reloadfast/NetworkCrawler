...
const device1: Device = {
  ...
  ports: [
    { id: 1, port_number: 22, protocol: 'tcp', service_name: 'ssh', version_banner: 'OpenSSH' },
    { id: 2, port_number: 80, protocol: 'tcp', service_name: 'http', version_banner: null },
  ],
}
...
const summary: RiskSummary = { critical: 1, high: 0, medium: 0, low: 0, total: 1 }
...
function buildFetch(overrides: Record<string, unknown> = {}) {
  return vi.fn((url: string) => {
    ...
    const match = Object.keys(defaults).find((k) => url === k || url.startsWith(k + '?'))
...
describe('DashboardPage', () => {
  ...
  it('shows trigger button and calls /api/scans/trigger on click', async () => {
    ...
    await waitFor(() => expect(screen.getByText('Trigger Scan')).toBeInTheDocument())
    fireEvent.click(screen.getByText('Trigger Scan'))
    await waitFor(() => expect(screen.getByText(/Scan #99 started/i)).toBeInTheDocument())
  })
})
...
describe('DevicesPage', () => {
  ...
  it('shows empty state when no devices', async () => {
    ...
    await waitFor(() => expect(screen.getByText(/No devices discovered yet/i)).toBeInTheDocument())
  })
  ...
  it('filters by search input', async () => {
    ...
    await waitFor(() => expect(screen.getByText('10.0.0.1')).toBeInTheDocument())
    fireEvent.change(screen.getByRole('searchbox'), { target: { value: '10.0.0.1' } })
  ...
  it('shows no match row when filter yields nothing', async () => {
    ...
    expect(screen.getByText(/No devices match the filter/i)).toBeInTheDocument()
  })
...
describe('DeviceDetailPage', () => {
  ...
  it('shows risk list for device', async () => {
    vi.stubGlobal('fetch', buildFetch())
    renderDetail('1')
    await waitFor(() => expect(screen.getByText('Default credentials')).toBeInTheDocument())
  })
...
describe('RisksPage', () => {
  ...
  it('opens risk detail modal on row click', async () => {
    ...
    await waitFor(() => expect(screen.getByText('Default credentials')).toBeInTheDocument())
    fireEvent.click(screen.getByRole('button', { name: /View details for Default credentials/i }))
