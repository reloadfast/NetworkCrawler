...
const mockSummary: RiskSummary = { critical: 0, high: 0, medium: 0, low: 0, total: 0 }
...
describe('ScanBanner', () => {
  it('renders with accessible role and label', () => {
    render(<ScanBanner />)
    expect(screen.getByRole('status', { name: /scan in progress/i })).toBeInTheDocument()
  })
...
describe('ToastContainer', () => {
  ...
  it('renders a toast message', () => {
    const toasts = [{ id: '1', message: 'Scan completed!', variant: 'success' as const }]
  ...
  it('calls onDismiss when dismiss button clicked', () => {
    ...
    const toasts = [{ id: 'abc', message: 'Test toast', variant: 'info' as const }]
...
describe('DashboardPage (polish)', () => {
  it('shows scan button enabled when no scan is running', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByRole('button', { name: /trigger a new scan/i })).not.toBeDisabled(),
    )
  })
  ...
  it('does not show ScanBanner when no scan is running', async () => {
    ...
    expect(screen.queryByRole('status', { name: /scan in progress/i })).not.toBeInTheDocument()
  })
  ...
  it('shows a toast after triggering a scan', async () => {
    ...
    const btn = await screen.findByRole('button', { name: /trigger a new scan/i })
    fireEvent.click(btn)
