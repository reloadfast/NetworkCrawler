...
def _migrate_schema(engine) -> None:
    ...
    with engine.connect() as conn:
        for table, column, col_def in migrations:
            result = conn.execute(text(f"PRAGMA table_info({table})"))  # noqa: S608 â PRAGMA not user input
            existing_cols = {row[1] for row in result}
            if column not in existing_cols:
                conn.execute(text(f"ALTER TABLE {table} ADD COLUMN {column} {col_def}"))  # noqa: S608 â table/column/col_def are internal constants, not user input
                conn.commit()
        # Ensure the MAC address index exists (CREATE INDEX IF NOT EXISTS is idempotent).
        conn.execute(
            text(
                "CREATE INDEX IF NOT EXISTS ix_devices_mac_address ON devices (mac_address)"  # noqa: S608 â DDL constant
            )
        )
        conn.commit()
...
def init_db() -> None:
    ...
    # Ensure the data directory exists â required when a volume is freshly mounted
    ...
    from app.models import device as _device  # noqa: F401 â side-effect import registers ORM tables
    from app.models import (
        recommendation as _recommendation,  # noqa: F401 â side-effect import registers ORM tables
    )
    from app.models import risk as _risk  # noqa: F401 â side-effect import registers ORM tables
    from app.models import scan as _scan  # noqa: F401 â side-effect import registers ORM tables
    from app.models import (
        scan_event as _scan_event,  # noqa: F401 â side-effect import registers ORM tables
    )
    from app.models import (
        settings as _settings,  # noqa: F401 â side-effect import registers ORM tables
    )
...
def upsert_device(
    ...
    *,
    ...
    1. If mac_address is provided, find an existing device with that MAC.
       - Found at the same IP â normal update (non-None fields only).
       - Found at a different IP â update ip_address to the new one, preserving
    ...
    if mac_address is not None:
        ...
        if device is not None and device.ip_address != ip_address:
            ...
            if existing_device_with_new_ip is not None:
                ...
                # Device moved to a new IP â update the address in place so
